...
let PREPARE_POOL_METADATA = async() => {
    ...
    for(let poolPubKey of poolsPubKeys){
        ...
        let myHealthStatusOfThisPool = await USE_TEMPORARY_DB('get',tempObject.DATABASE,'HEALTH:'+poolPubKey).catch(_=>false) // {wasSkipped:true}
        ...
        if(myMetadataForPoolForThisCheckpoint){
            ...
            if(myHealthStatusOfThisPool?.wasSkipped) tempObject.HEALTH_MONITORING.add(poolPubKey)
...
let GET_WSS_ADDRESS_AND_OPEN_CONNECTION = async (poolPubKey,primePoolsArray) => {
    ...
    if(poolOriginSubchain){
        ...
        if(possiblePoolData?.wssPoolURL){
            ...
            OPEN_WSS_CONNECTION_AND_START_ALL_PROCEDURES(poolPubKey,possiblePoolData.wssPoolURL,primePoolsArray)
            
        }else{
            
            LOG(`Can't get the WSS address for pool \u001b[38;5;154m ${poolPubKey}`,'FAIL')

            setTimeout(()=>GET_WSS_ADDRESS_AND_OPEN_CONNECTION(poolPubKey,primePoolsArray),2000)
    ...
    if(stillNoCheckpointOrNextDay){
        ...
        if(latestCheckpointOrError.completed && nextCheckpointFullID !== CURRENT_CHECKPOINT_ID){
            ...
            let tempObject = {
                ...
                POOLS_METADATA:new Map(), // poolPubKey => {index,hash,isReserve,currentAuthority(?),...}
                ...
                HEALTH_MONITORING:new Set(), // public keys of pools that was reassigned(if they were responsible for some subchain). We need this set to know if we should close WSS connection to this pool
